...
function App() {
      ...
      intervalId = setInterval(async () => {
        try {
          ...
          if (data.status === 'completed') {
            ...
            setTimeout(() => setJobStatus("MUTATING"), 1000);
          }
        } catch (err) {
  ...
  const handleLaunchAgent = async () => {
    try {
      ...
      setTimeout(() => setJobStatus("MUTATING"), 3000);
    }
  };
  ...
  const handleResetDashboard = () => {
    setJobId(null);
    setTargetUrl("https://staging.core-app.io/checkout");
    setJobStatus('IDLE');
  ...
  return (
    <div className="flex-1 flex overflow-hidden relative bg-[#040711] w-full h-screen text-slate-100 font-sans select-none">
        ...
        <div>
          ...
          <nav className="space-y-1">
            <a 
              ...
              className={`w-full flex items-center justify-between px-3 py-2.5 rounded-lg font-medium border transition duration-300 text-left ${activeTab === "dashboard" ? 'bg-slate-900/80 text-white border-white/5' : 'text-slate-400 border-transparent hover:bg-slate-900/50 hover:text-slate-200'}`}
            >
            ...
            <a 
              ...
              className={`w-full flex items-center justify-between px-3 py-2.5 rounded-lg font-medium border transition duration-300 text-left ${activeTab === "active-jobs" ? 'bg-slate-900/80 text-white border-white/5' : 'text-slate-400 border-transparent hover:bg-slate-900/50 hover:text-slate-200'}`}
            >
              <span className="flex items-center gap-3"><span>🔄</span> Active Jobs</span>
              {activeTab === "active-jobs" && <span className="w-1.5 h-1.5 bg-violet-400 rounded-full"></span>}
            </a>
            <a 
              href="#"
              onClick={(e) => { e.preventDefault(); setActiveTab("repositories"); }} 
              className={`w-full flex items-center justify-between px-3 py-2.5 rounded-lg font-medium border transition duration-300 text-left ${activeTab === "repositories" ? 'bg-slate-900/80 text-white border-white/5' : 'text-slate-400 border-transparent hover:bg-slate-900/50 hover:text-slate-200'}`}
            >
            ...
            <a 
              ...
              className={`w-full flex items-center justify-between px-3 py-2.5 rounded-lg font-medium border transition duration-300 text-left ${activeTab === "analytics" ? 'bg-slate-900/80 text-white border-white/5' : 'text-slate-400 border-transparent hover:bg-slate-900/50 hover:text-slate-200'}`}
            >
            ...
            <a 
              ...
              className={`w-full flex items-center justify-between px-3 py-2.5 rounded-lg font-medium border transition duration-300 text-left ${activeTab === "settings" ? 'bg-slate-900/80 text-white border-white/5' : 'text-slate-400 border-transparent hover:bg-slate-900/50 hover:text-slate-200'}`}
            >
              <span className="flex items-center gap-3"><span>⚙️</span> Agent Settings</span>
      ...
      {activeTab !== "dashboard" ? (
        /* Requirement 2: Dark slate placeholder layout container for other tabs */
        <main className="flex-1 flex flex-col items-center justify-center p-8 bg-slate-900/10 z-10 text-center">
          <div className="bg-slate-950/60 border border-white/5 p-8 rounded-xl max-w-lg shadow-2xl glow-glow space-y-4 backdrop-blur-md">
            <div className="text-4xl">⚙️</div>
            <h2 className="text-xl font-bold capitalize text-white">{activeTab.replace('-', ' ')}</h2>
            <p className="text-xs text-slate-400 leading-relaxed">This section is currently offline for sync. AuraHeal.AI core agents are running active visual layout audits on the primary dashboard channel.</p>
            <button 
              onClick={() => setActiveTab("dashboard")} 
              className="bg-violet-600 hover:bg-violet-500 text-white text-xs font-semibold px-5 py-2.5 rounded shadow-lg shadow-violet-600/20 transition-all duration-300 cursor-pointer"
            >
              Return to Dashboard
            </button>
          </div>
                ...
                <div className="max-w-6xl w-full mx-auto p-8 grid grid-cols-1 lg:grid-cols-3 gap-8 my-auto">
                  <div className="lg:col-span-2 space-y-6">
                    <div>
                      ...
                      <p className="text-slate-400 text-sm">Initiate an autonomous visual health pass and self-healing analysis across your application environments.</p>
                    </div>
                    ...
                    <div className="bg-slate-950/60 border border-white/5 backdrop-blur-md rounded-xl p-6 shadow-2xl glow-glow space-y-6">
                      ...
                      <div className="pt-2">
                        <button 
                          ...
                          className="w-full bg-gradient-to-r from-violet-600 to-indigo-600 hover:from-violet-500 hover:to-indigo-500 text-white font-semibold py-4 rounded-lg shadow-[0_0_30px_rgba(139,92,246,0.25)] transition-all duration-300 transform hover:-translate-y-0.5 flex items-center justify-center gap-2 cursor-pointer"
                        >
                ...
                <header className="h-16 border-b border-white/5 bg-slate-950/40 backdrop-blur px-6 flex items-center justify-between shrink-0">
                  ...
                  <div className="flex items-center gap-2">
                    <button 
                      ...
                      className="bg-slate-900/60 hover:bg-slate-800 text-slate-300 text-xs font-medium px-3 py-1.5 rounded border border-white/5 transition-all cursor-pointer"
                    >
                      🛑 Cancel
                    </button>
                  </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-6 items-start">
                    <div className="bg-slate-950/50 border border-white/5 rounded-xl overflow-hidden shadow-xl backdrop-blur-sm">
                      <div className="bg-slate-900/80 px-4 py-2 border-b border-white/5 text-xs text-slate-400 flex items-center justify-between">
                        ...
                        <span className="bg-emerald-500/10 text-emerald-400 text-[10px] px-2 py-0.5 rounded border border-emerald-500/20 flex items-center gap-1"><span className="w-1 h-1 bg-emerald-400 rounded-full animate-pulse"></span>Scanning</span>
                      </div>
            ...
            <div className="absolute inset-0 flex flex-col overflow-y-auto p-6 space-y-6 transition-all duration-300 z-10 max-w-6xl mx-auto justify-center w-full h-full">
              ...
              <footer className="bg-slate-950/40 border border-white/5 backdrop-blur-md rounded-xl p-4 flex flex-col sm:flex-row items-center justify-between shadow-xl gap-4 shrink-0">
                ...
                <button 
                  ...
                  className="w-full sm:w-auto bg-gradient-to-r from-violet-600 to-indigo-600 hover:from-violet-500 hover:to-indigo-500 text-white text-xs font-bold px-6 py-3 rounded-lg shadow-lg shadow-indigo-600/20 transition-all duration-300 cursor-pointer animate-pulse"
                >
          ...
          {jobStatus === 'SUCCESS' && (
            <div className="absolute inset-0 flex flex-col overflow-y-auto p-6 items-center justify-center transition-all duration-300 z-10 w-full h-full">
              <div className="max-w-4xl w-full space-y-6 my-auto">
                ...
                <div className="bg-[#0a0d14] border border-white/5 rounded-xl overflow-hidden shadow-2xl">
                  <div className="bg-[#11151f] px-4 py-3 border-b border-white/5 flex items-center justify-between text-xs">
                    <span className="font-semibold text-slate-200">Pull Request #121: <span className="font-normal text-slate-400 ml-1">Fix mobile hero layout layout padding constraints</span></span>
                    <span className="bg-[#2da44e]/10 text-[#2da44e] px-2.5 py-0.5 rounded-full border border-[#238636]/30 font-bold text-[11px]">Open</span>
                  </div>
                  <div className="p-5 space-y-4">
                    <p className="text-xs text-slate-400 leading-relaxed">AuraHeal AI Engine successfully updated local CSS utility vectors. Core regression unit testing suites passed across repository deployment trees flawlessly.</p>
                    <div className="bg-[#11151f] border border-white/5 rounded-lg p-3 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                      <span className="text-xs font-mono text-indigo-400 bg-indigo-950/30 px-2 py-1 rounded border border-indigo-900/30 code-font">{activeBranch || `auraheal/fix-${targetBranch}`}</span>
                      <div className="flex gap-2 w-full sm:w-auto">
                        <button 
                          onClick={() => alert('Deployment merged successfully via Pull Request!')} 
                          className="bg-[#238636] hover:bg-[#2ea043] text-white font-bold text-xs px-4 py-2.5 rounded-md shadow-md transition-all cursor-pointer w-full sm:w-auto text-center"
                        >
                          Merge Patch via AuraHeal Agent
                        </button>
                        <button 
                          onClick={handleResetDashboard}
                          className="bg-slate-800 hover:bg-slate-700 text-slate-200 border border-white/5 font-bold text-xs px-4 py-2.5 rounded-md shadow-md transition-all cursor-pointer w-full sm:w-auto text-center"
                        >
                          Reset Dashboard
                        </button>
                      </div>
